    ...
    return (
        <div className="flex flex-col items-center">
            <svg width={100} height={100} viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r={r} fill="none" stroke="#00B4D8" strokeWidth="8" strokeLinecap="round"
                    strokeDasharray={`${(score / 100) * circ} ${circ}`} transform="rotate(-90 50 50)" />
                <text x="50" y="54" textAnchor="middle" fontSize="18" fontWeight="900" fill="#0A2342">{score}</text>
            </svg>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest -mt-1">Discovery Score</span>
        </div>
    );
};
            ...
            <div className="bg-white rounded-2xl max-w-md w-full p-8 shadow-2xl" onClick={e => e.stopPropagation()}>
                {sent ? (
                    <div className="text-center py-6">
                        ...
                        <h3 className="text-xl font-black text-[#0A2342] mb-2">Sent Successfully</h3>
                        <p className="text-[#4A4A6A] text-sm">Your personalised blueprint for {orgName} is on its way. A Pithonix architect will follow up within 24 hours.</p>
                        <button onClick={onClose} className="mt-6 bg-[#0A2342] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#00B4D8] transition-all">Close</button>
                    </div>
                ) : (
                    <>
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h3 className="text-xl font-black text-[#0A2342]">Get Your Full Blueprint</h3>
                                <p className="text-sm text-[#4A4A6A] mt-1">Receive your complete AI diagnostic report personalised for {orgName}.</p>
                            </div>
                            <button onClick={onClose} className="text-slate-400 hover:text-[#0A2342]"><X size={20} /></button>
                        </div>
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={field}>
                                    <label className="text-xs font-black text-[#4A4A6A] uppercase tracking-widest mb-1 block">{label}</label>
                                    <input type={type} value={form[field]} onChange={e => setForm(p => ({ ...p, [field]: e.target.value }))} placeholder={placeholder}
                                        className="w-full border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#00B4D8] transition-all" />
                                </div>
                            ))}
                            <div>
                                <label className="text-xs font-black text-[#4A4A6A] uppercase tracking-widest mb-1 block">Primary Challenge (optional)</label>
                                <textarea value={form.challenge} onChange={e => setForm(p => ({ ...p, challenge: e.target.value }))} placeholder="Describe your biggest operational challenge..."
                                    rows={2} className="w-full border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#00B4D8] transition-all resize-none" />
                            </div>
                        </div>
                        <button onClick={handleSubmit} disabled={!isValid || sending}
                            className={`w-full mt-6 py-3 rounded-xl font-black text-base flex items-center justify-center gap-2 ${isValid ? 'bg-[#0A2342] text-white hover:bg-[#00B4D8]' : 'bg-slate-200 text-slate-400 cursor-not-allowed'} transition-all`}>
...
const SimulatorScreen5 = ({ simState, setSimState, onAddUseCase }) => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center h-[500px] space-y-6">
            <Trophy className="animate-bounce text-[#F4A261]" size={64} />
            <h2 className="text-3xl font-black text-[#0A2342]">Simulating Final Outcomes...</h2>
            <p className="text-[#4A4A6A]">Running value calculations for {orgName}</p>
        </div>
    );
    ...
    return (
        <div id="screen5-content" className="space-y-10 animate-fadeIn max-w-5xl mx-auto pb-24">
            ...
            <div className="text-center space-y-2">
                <Sparkles className="mx-auto text-[#00B4D8]" size={40} />
                <h2 className="text-4xl font-black text-[#0A2342] uppercase tracking-tight">
                    Your Complete Intelligence Outcome — Simulation {simCount}
                </h2>
                <p className="text-[#4A4A6A]">
                    <strong>{orgName}</strong> · {industry} · {size}
                </p>
            </div>
            ...
            {simState.currentImpact && (
                <div className="border-l-4 border-[#F4A261] bg-[#FFFBF5] rounded-xl p-5">
                    <div className="text-[10px] font-black text-[#F4A261] uppercase tracking-widest mb-1">You Told Us</div>
                    <p className="text-sm font-semibold text-[#0A2342] italic">"{simState.currentImpact}"</p>
                    <p className="text-xs text-[#4A4A6A] mt-2">Here is our analysis, calibrated to your organisation.</p>
                </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    { label: 'Annual Value at Risk', value: valueCalcs ? `₹${formatLakhs(valueCalcs.currentAnnualPain?.total)}` : (data?.estimatedROI?.annualValue || 'Calculating...'), color: '#0A2342' },
                    ...
                    { label: 'Payback Period', value: valueCalcs?.paybackWeeks ? `${valueCalcs.paybackWeeks} weeks` : '12–18 weeks', color: '#00B4D8' }
                ].map((m, i) => (
                    <div key={i} className="bg-[#0A2342] rounded-xl p-5 text-center">
                        <div className="text-2xl font-black" style={{ color: m.color === '#0A2342' ? '#F4A261' : m.color }}>{m.value}</div>
                        <div className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">{m.label}</div>
                    </div>
                ))}
            </div>
            ...
            <div className="space-y-3">
                <h4 className="text-xs font-black text-[#4A4A6A] uppercase tracking-widest">5-Layer Outcome View</h4>
                {PANELS.map((p, i) => {
                    const pd = data?.[p.key] || {};
                    const isActive = activePanel === i;
                    return (
                        <div key={p.key} className={`sim-card overflow-hidden transition-all duration-300 ${isActive ? 'shadow-lg' : ''}`}>
                            <button onClick={() => setActivePanel(isActive ? -1 : i)} className="w-full flex items-center justify-between p-5 text-left">
                                <div className="flex items-center gap-3">
                                    <span className="text-xl">{p.icon}</span>
                                    <span className="font-black text-[#0A2342]">{p.label}</span>
                                </div>
                                {isActive ? <ChevronUp size={18} className="text-[#4A4A6A]" /> : <ChevronDown size={18} className="text-[#4A4A6A]" />}
                            </button>
                            {isActive && (
                                <div className="px-6 pb-6 space-y-4 animate-fadeIn">
                                    <p className="text-sm text-[#4A4A6A] leading-relaxed">{pd.narrative || bench.outcomeNarratives?.[p.key]}</p>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                                            {pd.metrics.map((m, mi) => (
                                                <div key={mi} className="text-center">
                                                    ...
                                                    <div className="text-xs text-slate-500 line-through">{m.before}</div>
                                                    <div className="text-sm font-black text-green-600">{m.after}</div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
            ...
            <div className="border-t border-b border-slate-200 py-5 flex items-center justify-between">
                <p className="text-sm text-[#4A4A6A]">Simulation {simCount} complete · {useCases.length} use case{useCases.length !== 1 ? 's' : ''} analysed</p>
                <button onClick={onAddUseCase}
                    className="text-sm font-black text-[#00B4D8] hover:underline">
            ...
            {/* ─── 8. CTA SECTION ──────────────────────────────────────────── */}
            <div className="bg-[#0A2342] rounded-2xl p-8 text-center space-y-4">
                <h3 className="text-2xl font-black text-white uppercase">Request Intelligence Blueprint</h3>
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-center mt-2">
                    <button
                        ...
                        disabled={pdfLoading}
                        className="bg-[#00B4D8] text-white px-8 py-4 rounded-2xl font-black text-lg hover:bg-white hover:text-[#0A2342] transition-all flex items-center justify-center gap-3 disabled:opacity-70"
                    >
                        {pdfLoading ? <><Loader2 className="animate-spin" size={20} /> Generating...</> : <><FileText size={20} /> Download Simulation Report</>}
                    </button>
                    <button
                        onClick={() => setIsModalOpen(true)}
                        className="border-2 border-[#00B4D8] text-[#00B4D8] px-8 py-4 rounded-2xl font-black text-lg hover:bg-[#00B4D8] hover:text-white transition-all flex items-center justify-center gap-3"
                    >
