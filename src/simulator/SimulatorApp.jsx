...
const SimulatorApp = () => {
    ...
    return (
        <SimDemoModeWrapper>
            <div className="simulator-container h-screen flex flex-col bg-[#F8FAFC] overflow-hidden">
                <header className="bg-white border-b p-6 flex-shrink-0 sticky top-0 z-50">
                    <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center gap-6">
                        <div className="flex items-center gap-4">
                            {currentStep > 1 && currentStep < 5 && (
                                <button
                                    onClick={prevStep}
                                    className="p-2 hover:bg-slate-100 rounded-full transition-all text-[#0A2342]"
                                >
                                    <ArrowLeft size={24} />
                                </button>
                            )}
                            <h1 className="text-2xl font-black text-[#0A2342] tracking-tight uppercase">
                                Pithonix <span className="text-[#00B4D8]">Discovery</span>
                            </h1>
                        </div>
                        ...
                        {currentStep === 2 && simState.blueprintReady && (
                            <button
                                onClick={nextStep}
                                className="bg-[#0A2342] hover:bg-[#153457] text-white px-5 py-2.5 rounded-xl font-black flex items-center gap-2 transition-all shadow-lg hover:-translate-y-0.5 active:scale-95 animate-pulse-subtle text-xs"
                            >
                ...
                {currentStep < 5 && (
                    <footer className="max-w-6xl w-full mx-auto px-6 py-4 flex-shrink-0 text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center justify-center gap-4">
                        <span>Pithonix Global Enterprise Intelligence</span>
                        <span className="w-1 h-1 bg-slate-400 rounded-full" />
                        <span className="text-[#00B4D8]">Diagnostic Engine: Gemini 2.5 Flash-Lite</span>
                    </footer>
                )}
            </div>
        </SimDemoModeWrapper>
